...
## Direction hints
- **Discover surface**: `netstat -ap | grep LISTENING` or `/proc/net/unix` reveals socket services; `service list` + `lshal` reveals Binder/HIDL services
- **Socket types**: Abstract socket (`@`-prefixed, no filesystem) and File System socket (filesystem path) — functionally identical for exploitation
...
- **Common vuln types in native services**: command injection, path traversal, integer overflow in length fields, OOB read/write from unchecked attacker-controlled sizes, SELinux misconfiguration
