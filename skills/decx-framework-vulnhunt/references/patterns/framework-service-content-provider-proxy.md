...
Consider safe when authorities are immutable allowlisted, URI paths are canonicalized, provider permissions are enforced for the original caller, user/profile boundaries are checked, and grants are limited to trusted recipients.
...
Use: "A Binder-exposed framework method proxies attacker-selected provider access under privileged service identity without enforcing caller-scoped authorization."
