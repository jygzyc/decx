...
- Reachability: attacker can obtain, trigger, or influence the pending intent path.
- Controllability: attacker controls action, data, extras, component, mutability, or fill-in fields used by the sink.
- Sink: pending intent execution performs protected work, launches private components, returns/grants sensitive data, or bypasses normal caller checks.
- Missing or bypassable guard: no immutable trusted mapping, exact target pinning, or validation at execution time.
- Visible impact: victim-identity action, privilege reuse, data exposure, or meaningful cross-app workflow bypass.

## Guard Checklist

Consider safe when pending intents are immutable, target explicit trusted components, use stable non-attacker request codes where needed, ignore untrusted fill-in fields, and repeat authorization at execution time.
...
Use: "The app creates or accepts a pending intent whose attacker-controlled fields execute a protected action under the app identity."

Avoid: "PendingIntent is created with caller-controlled content" without proving the PendingIntent executes under victim identity with security impact.
