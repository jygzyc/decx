...
- Reachability: external entry reaches the forwarding component.
- Controllability: attacker controls the forwarded `Intent` or security-relevant fields.
- Sink: the forwarded object reaches a concrete launch, result, broadcast, service, or grant sink.
- Missing or bypassable guard: no exact component/package/signature/caller allowlist blocks the path.
- Visible impact: private component access, sensitive data/grant exposure, privileged action, credential/session abuse, or meaningful chain pivot.

## Guard Checklist

Consider safe when validation occurs before forwarding and pins the exact trusted target, strips dangerous flags/grants/selector/ClipData, or verifies caller/package/signature with immutable allowlists. Action-only, scheme-only, package-prefix, or post-launch checks are not enough by themselves.
...
Use: "An exported Activity forwards attacker-controlled nested Intent data to a privileged component launch without exact target validation."

Avoid: "Intent redirect exists" without source, sink, guard, and impact evidence.
