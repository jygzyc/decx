...
## Non-obvious
- `applyBatch`/`bulkInsert`/`call` **routinely bypass** the per-operation authorization that `query`/`insert`/`update`/`delete` enforce — guard skew, not missing guard
...
- `FLAG_GRANT_PERSISTABLE_URI_PERMISSION` + `takePersistableUriPermission` = persistent grant surviving activity lifetime
