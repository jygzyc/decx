...
.small{font-size:12px;padding:5px 8px}.icon{width:32px;height:32px;padding:0;text-align:center}
...
.pill{display:inline-flex;align-items:center;border:1px solid #cfd8e6;border-radius:999px;padding:2px 8px;font-size:12px;background:#fff}
.active-dot{width:8px;height:8px;border-radius:50%;background:#22c55e;margin-right:6px}
.stopped-dot{width:8px;height:8px;border-radius:50%;background:#94a3b8;margin-right:6px}
.failed-dot{width:8px;height:8px;border-radius:50%;background:#ef4444;margin-right:6px}
.completed-dot{width:8px;height:8px;border-radius:50%;background:#1f6feb;margin-right:6px}
...
<body>
<header><div><h1>Agent Audit</h1><div class="sub">Graph-based agent blackboard</div></div><button class="small" onclick="refresh()">Refresh</button></header>
<main>
  <aside>
...
function tabs(){
  const names = [['overview','Overview'],['graph','Graph'],['intents','Intents'],['raw','Raw']];
...
function overview(d){
  return '<div class="metric-grid">' +
    ...
    metric('Links', d.links.length) + metric('Runs', d.runs.length) +
    '</div><div class="row" style="align-items:flex-start;margin-top:12px">' +
    '<form class="panel" style="flex:1" onsubmit="addLink(event, \\''+d.project.id+'\\')"><h3>Add Link</h3>' +
    '<div class="field"><label class="label">From fact</label><input id="link-from-'+d.project.id+'" placeholder="f001"></div>' +
    '<div class="field"><label class="label">To fact</label><input id="link-to-'+d.project.id+'" placeholder="f002"></div>' +
    '<div class="field"><label class="label">Kind</label><input id="link-kind-'+d.project.id+'" placeholder="enables"></div>' +
    '<button class="primary" type="submit">Add Link</button></form>' +
    '<form class="panel" style="flex:1" onsubmit="addIntent(event, \\''+d.project.id+'\\')"><h3>New Intent</h3>' +
    '<div class="field"><textarea id="intent-desc-'+d.project.id+'" placeholder="One concrete next task"></textarea></div>' +
    '<div class="row"><input id="intent-from-'+d.project.id+'" placeholder="from fact id (or origin)"><select id="intent-worker-'+d.project.id+'"><option value="">project worker</option><option>noop</option><option>codex</option><option>claude-code</option><option>opencode</option><option>api</option></select></div>' +
    '<button class="primary" type="submit">Create Intent</button></form>' +
    '</div><div style="margin-top:12px">'+recentActivity(d)+'</div>';
}
function graphView(d){
  return '<div class="graph">' +
    lane('Facts', d.facts.map(f => node('fact', f.id, (f.confidence<1?(f.confidence*100).toFixed(0)+'% ':'')+f.description, f))) +
    lane('Intents', d.intents.map(i => node('intent', i.id, '['+i.status+'] '+(i.fromFacts.join(',')||'origin')+' → '+(i.to||'?')+' '+i.description, i))) +
    lane('Links', d.links.map(l => node('link', l.id, l.fromFactId+' --'+l.kind+'--> '+l.toFactId, l))) +
    lane('Runs', d.runs.map((r,idx) => node('run', String(idx+1), r.phase+'/'+r.worker+' code='+r.returncode, r))) +
    '</div>';
}
...
function recentActivity(d){
  const runs = d.runs.slice(-6).reverse().map(r=>'<div class="item"><strong>'+r.phase+'/'+r.worker+'</strong> code='+r.returncode+' <span class="muted">'+escapeHtml(r.stdoutPreview.slice(0,80))+'</span></div>').join('');
  return '<div class="panel"><h3>Recent runs</h3>'+(runs || '<div class="muted">none</div>')+'</div>';
}
...
function findItem(kind,id){
  ...
  const map = {fact:current.facts,intent:current.intents,link:current.links};
  if (kind === 'run') return {kind:kind,id:id,data:current.runs[Number(id)-1]};
...
async function addLink(event,id){
  event.preventDefault();
  const from = document.getElementById('link-from-'+id);
  const to = document.getElementById('link-to-'+id);
  const kind = document.getElementById('link-kind-'+id);
  if (!from.value.trim() || !to.value.trim()) return;
  await fetch('/api/projects/'+encodeURIComponent(id)+'/links',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({fromFactId:from.value,toFactId:to.value,kind:kind.value||'related'})});
  from.value=''; to.value=''; kind.value='';
  await loadDetail(id);
}
async function addIntent(event,id){
  event.preventDefault();
  const desc = document.getElementById('intent-desc-'+id);
  const from = document.getElementById('intent-from-'+id);
  const worker = document.getElementById('intent-worker-'+id);
  if (!desc.value.trim()) return;
  const body = {description:desc.value,creator:'web-ui',from:from.value.trim()?[from.value.trim()]:['origin']};
