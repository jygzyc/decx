...
object McpToolRegistry {
    ...
    val tools: List<McpTool> = listOf(healthTool()) + commonTools() + contextTools() + androidTools() + uiTools() + vulnTools()
    ...
    // ---------------------------------------------------------------------
    // Vuln hunt tools (Tai-e evidence collection) — server mode with --tai-e only.
    // These tools collect structured evidence for AI vulnerability investigation.
    // The engine does NOT make vulnerability judgments — the AI does.
    // ---------------------------------------------------------------------
    ...
    private fun vulnTools(): List<McpTool> = listOf(
        ...
        routeTool(
            ...
            toPayload = { args -> linkedMapOf("mth" to methodArg(args), "var" to (args["var"] ?: args["variable"] ?: "return"), "page" to pageArg(args)) }
        ),
        noArgRoute(
        ...
        routeTool(
            ...
            required = listOf("mth"),
            toPayload = { args -> linkedMapOf("mth" to methodArg(args), "direction" to (args["direction"] ?: "both"), "page" to pageArg(args)) }
        )
    )
